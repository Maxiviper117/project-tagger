...
function getTagsFolder(basePath: string): string {
    ...
    if (existsSync(oldTagsPath)) {
        ...
        console.log(
            chalk.green(`✅ Migrated tags from 'tags' to '${TAGS_FOLDER}'.`)
        );
    }
...
function addTag(tagsPath: string, tag: string): void {
    ...
    if (!existsSync(tagFile)) {
        ...
        console.log(
            chalk.green(`✅ Tag '${sanitizedTag}' added successfully.`)
        );
    } else {
...
function listTags(tagsPath: string): void {
    try {
        ...
        console.error(
            chalk.red(`❌ Error listing tags: ${(err as Error).message}`)
        );
    }
}
    ...
    while (true) {
        const { action } = await inquirer.prompt({
            ...
            choices: [
                ...
                {
                    name: "🔍 Detect Project Type and Suggest Tags",
                    value: "detect",
                }, // New option
            ...
            case "add":
                ...
                if (addMethod === "custom") {
                    const { customTag } = await inquirer.prompt<{
                        ...
                        name: "customTag",
                        message:
                            "Enter the names of the custom tags, separated by commas:",
                    });
...
async function detectAndSuggestTags(
    tagsPath: string,
    basePath: string
): Promise<void> {
    ...
    if (
        ...
        existsSync(resolve(basePath, "public"))
    ) {
    ...
    if (
        existsSync(resolve(basePath, "requirements.txt")) ||
        existsSync(resolve(basePath, "setup.py"))
    ) {
    ...
    if (
        (existsSync(resolve(basePath, "app.js")) ||
            existsSync(resolve(basePath, "server.js"))) &&
        existsSync(packageJsonPath) &&
    ...
    if (
        ...
        existsSync(resolve(basePath, "src")) &&
        readdirSync(resolve(basePath, "src")).some((file) =>
    ...
    if (suggestedTags.length === 0) {
        console.log(
            chalk.yellow(
                "❌ No recognizable project files found for auto-suggesting tags."
            )
        );
        return;
    }
